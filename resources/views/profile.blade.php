...
@section('content')
    <div class="container mt-4" style="max-width: 600px;">
        ...
        @if (session('2fa:qrCode') && !auth()->user()->two_factor_secret)
            <div class="mb-4">
                ...
                {!! session('2fa:qrCode') !!}
                <p><strong>Secret Key:</strong> {{ session('2fa:secret') }}</p>

                <form method="POST" action="{{ route('2fa.enable') }}">
                    @csrf
                    <div class="mb-3">
                        <label class="form-label">Enter Authenticator Code:</label>
        ...
        @if (!auth()->user()->two_factor_secret && !session('2fa:qrCode'))
            <form action="{{ route('2fa.setup') }}" method="POST" class="mb-4">
                @csrf
                <button type="submit" class="btn btn-success w-100">Enable Two-Factor Authentication</button>
            </form>
        @endif
    ...
    <div id="authenticatorModal" class="modal fade" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered" style="max-width: 400px;">
            <form method="POST" action="{{ route('2fa.disable.auth') }}" class="modal-content">
                @csrf
                ...
                <div class="modal-body">
                    ...
                    <p class="text-center mt-3">
                        <a href="#" onclick="switchToRecovery()">Use recovery code instead</a>
                    </p>
                </div>
            </form>
        </div>
    ...
    <div id="recoveryModal" class="modal fade" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered" style="max-width: 400px;">
            <form method="POST" action="{{ route('2fa.disable.recovery') }}" class="modal-content">
                @csrf
                <div class="modal-header">
                ...
                <div class="modal-body">
                    ...
                    <button type="submit" class="btn btn-danger w-100">Disable</button>
                    <p class="text-center mt-3">
                        <a href="#" onclick="switchToAuthenticator()">Use authenticator code instead</a>
                    </p>
                </div>
            </form>
        </div>
    ...
    <div id="regenerateModal" class="modal fade" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered" style="max-width: 400px;">
            <form method="POST" action="{{ route('2fa.recovery') }}" class="modal-content">
                @csrf
                <div class="modal-header">

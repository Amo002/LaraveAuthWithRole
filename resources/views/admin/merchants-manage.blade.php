...
@section('content')
    <div class="container mt-4">
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <form
                                                ...
                                                @csrf
                                                <div class="mb-2">
                                                    <label for="permission_ids_{{ $role->id }}"
        ...
        @can('dev')
            @if (session('show_merchant_permissions'))
                <!-- Merchant-Specific Permissions Card -->
                <div class="card mb-4">
                        ...
                        <form action="{{ route('admin.merchants.permissions.store', $merchant->id) }}" method="POST"
                            ...
                            <div class="col-auto">
                                <input type="text" name="permission_name" class="form-control"
                                    placeholder="New Permission Name" required>
                            </div>
                            <div class="col-auto">
                ...
                @else
                    <!-- Floating Button to Open Dev Modal -->
                    <button class="btn btn-sm btn-dark position-fixed" style="bottom: 20px; right: 20px; z-index: 1050;"
                        data-bs-toggle="modal" data-bs-target="#devPermissionModal">
                        Dev Access
                    </button>
                @endif
            @endcan
    ...
    <!-- Dev Permission Modal -->
    @can('dev')
        <div class="modal fade" id="devPermissionModal" tabindex="-1" aria-labelledby="devPermissionModalLabel"
            aria-hidden="true">
            <div class="modal-dialog modal-sm modal-dialog-bottom-end position-fixed bottom-0 end-0 m-3">
                <form action="{{ route('admin.merchants.permissions.unlock', $merchant->id) }}" method="POST"
                    class="modal-content border border-dark shadow-sm">
                    @csrf
                    <div class="modal-header py-2">
                        <h6 class="modal-title" id="devPermissionModalLabel">Developer Access</h6>
                        <button type="button" class="btn-close btn-close-sm" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body py-2">
                        <input type="password" name="password" class="form-control form-control-sm"
                            placeholder="Enter Dev Password" required>
                    </div>
                    <div class="modal-footer py-2">
                        <button type="submit" class="btn btn-sm btn-primary w-100">Unlock</button>
                    </div>
                </form>
            </div>
        </div>
